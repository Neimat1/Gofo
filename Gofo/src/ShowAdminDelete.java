...
/**
 * this class to delete playground that admin don't approve it
 * * Date:10-jun-2020
 * @author noura,eman,alaa,neimat
 * @version 1.0
 *
 */
public class ShowAdminDelete {
	...
	/**
	 * function to delete not approved playgrounds
	 */
	public ShowAdminDelete()
	{
		...
		for(int i=0;i<play.size();i++)
		{
			...
			if(playi.getPlaygroundAvailbility().equals((AvailabilityStatus.ACTIVE))|| playi.getPlaygroundAvailbility().equals((AvailabilityStatus.SUSPENDED)))
			{
	    ...
	    btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				play.get(index).setPlaygroundAvailbility(AvailabilityStatus.DELETED);
				play.storePlaygrounds();
				indeces.removeElementAt(combo1.getSelectedIndex());
				combo1.removeItemAt(combo1.getSelectedIndex());
				JOptionPane.showMessageDialog(null,AvailabilityStatus.DELETED ,"", JOptionPane.INFORMATION_MESSAGE);
			}
	    });
    	frame.setTitle("Delete_PLayground");
